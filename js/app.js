...
function initFlowerPicker() {
  ...
  if (oldFlowerSelect) {
    oldFlowerSelect.addEventListener("change", function () {
      comboInput.value = oldFlowerSelect.value;
      renderColorOptions();
    });
  }
...
/* === v1.1.0 add: 大區塊折疊 + 右側懸浮快速切換 === */
function quickSwitchSection(sectionId){
  const navBtn = Array.from(document.querySelectorAll('.main-nav .nav-btn, .main-nav button')).find(function(btn){
    return (btn.getAttribute('onclick') || '').includes("'" + sectionId + "'") || (btn.getAttribute('onclick') || '').includes('"' + sectionId + '"');
  });
  if (typeof showSection === 'function') {
    showSection(sectionId, navBtn || document.querySelector('.main-nav .nav-btn'));
  }
  document.querySelectorAll('.quick-switch button').forEach(function(btn){ btn.classList.remove('active'); });
  const quickBtn = Array.from(document.querySelectorAll('.quick-switch button')).find(function(btn){
    return (btn.getAttribute('onclick') || '').includes("'" + sectionId + "'") || (btn.getAttribute('onclick') || '').includes('"' + sectionId + '"');
  });
...
function initCollapsibleBlocks(){
  document.querySelectorAll('.collapsible-block').forEach(function(block){
    ...
    row.className = 'collapse-head-row';
    block.insertBefore(row, first);
    ...
    btn.addEventListener('click', function(){
      block.classList.toggle('collapsed-block');
      btn.textContent = block.classList.contains('collapsed-block') ? '展開 ▼' : '收合 ▲';
    });
    row.appendChild(btn);
  ...
  document.querySelectorAll('.collapsible-title').forEach(function(title){
    if (title.dataset.collapseReady === '1') return;
    const h = title.querySelector('h2') || title.firstElementChild;
    if (!h) return;
    const row = document.createElement('div');
    row.className = 'collapse-head-row';
    title.insertBefore(row, h);
    row.appendChild(h);
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'collapse-toggle';
    btn.textContent = '收合 ▲';
    btn.addEventListener('click', function(){
      const target = document.getElementById(title.dataset.collapseTarget);
      if (!target) return;
      target.classList.toggle('collapsed-target');
      btn.textContent = target.classList.contains('collapsed-target') ? '展開 ▼' : '收合 ▲';
    });
    row.appendChild(btn);
...
window.addEventListener('DOMContentLoaded', initCollapsibleBlocks);
window.addEventListener('load', function(){
  ...
  if (active) {
    document.querySelectorAll('.quick-switch button').forEach(function(btn){ btn.classList.remove('active'); });
    const quickBtn = Array.from(document.querySelectorAll('.quick-switch button')).find(function(btn){
      return (btn.getAttribute('onclick') || '').includes("'" + active.id + "'");
    });
    if (quickBtn) quickBtn.classList.add('active');
  }
});
